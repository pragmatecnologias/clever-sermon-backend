...
function createSpanishMetadata() {
    ...
    var spanishEpubs = files.filter(function (f) { return f.startsWith('es_') && f.endsWith('.epub'); });
    console.log("\uD83D\uDCDA Found ".concat(spanishEpubs.length, " Spanish EPUB files\n"));
    var books = spanishEpubs.map(function (filename) {
        // Extract book code from filename (e.g., "es_PP(PP).epub" -> "PP")
        var match = filename.match(/^es_([^(\.]+)/);
        var code = match ? match[1] : filename.replace('es_', '').replace('.epub', '');
        var info = SPANISH_BOOK_INFO[code] || {
            title: code,
            category: 'Other'
        };
        var stats = fs.statSync(path.join(DOWNLOAD_DIR, filename));
        return {
            code: code,
            title: info.title,
            filename: filename,
    ...
    console.log("\uD83D\uDCC1 Saved to: ".concat(METADATA_FILE, "\n"));
    // Show summary by category
    var byCategory = books.reduce(function (acc, book) {
    ...
    Object.entries(byCategory)
        ...
        console.log("   ".concat(category, ": ").concat(count));
    });
}
