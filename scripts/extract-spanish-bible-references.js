...
var INPUT_FILE = path.join(__dirname, '../data/egw-parsed-spanish/paragraphs.json');
var OUTPUT_FILE = path.join(__dirname, '../data/egw-parsed-spanish/paragraphs-with-references.json');
...
function extractBibleReferences(text) {
    ...
    var pattern = new RegExp("(".concat(Object.keys(SPANISH_BOOKS).join('|'), ")\\s+(\\d+)(?::(\\d+)(?:[-\u2013](\\d+))?)?"), 'gi');
    var match;
    while ((match = pattern.exec(text)) !== null) {
        var spanishBook = match[1];
        var englishBook = SPANISH_BOOKS[spanishBook] || spanishBook;
