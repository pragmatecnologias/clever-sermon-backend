...
var outputPath = process.argv[2] || 'data/word-occurrences.json';
...
(function () { return __awaiter(void 0, void 0, void 0, function () {
    var resolvedOutput, tempPath, content, data, occurrences, _loop_1, _i, data_1, book;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
            ...
            case 1:
                _a.sent();
                tempPath = "".concat(resolvedOutput, ".json");
            ...
            case 3:
                ...
                _loop_1 = function (book) {
                    var abbrev = (book.abbrev || '').toLowerCase();
                    var bookName = book.name || bookMap[abbrev] || abbrev;
                    var chapters = book.chapters || [];
                    chapters.forEach(function (verses, chapterIndex) {
                        verses.forEach(function (verseText, verseIndex) {
                            var reference = "".concat(bookName, " ").concat(chapterIndex + 1, ":").concat(verseIndex + 1);
                            var words = verseText.split(/\s+/g).map(normalizeWord).filter(Boolean);
                            var unique = new Set(words);
                            unique.forEach(function (word) {
                                if (!occurrences[word]) {
                                    occurrences[word] = { count: 0, verses: [] };
                                }
                                occurrences[word].count += words.filter(function (item) { return item === word; }).length;
                ...
                return [4 /*yield*/, fs_1.promises.writeFile(resolvedOutput, "".concat(JSON.stringify(occurrences, null, 2), "\n"))];
            case 4:
                _a.sent();
                return [4 /*yield*/, fs_1.promises.unlink(tempPath)];
            case 5:
                _a.sent();
                console.log("Saved word occurrences to ".concat(resolvedOutput));
                return [2 /*return*/];
        }
    });
}); })();
