...
const OUTPUT_DIR = path.join(__dirname, '../data/egw-parsed-spanish');
    ...
    for (const entry of zipEntries) {
      if (entry.entryName.endsWith('.xhtml') || entry.entryName.endsWith('.html')) {
        ...
        if (text.length > 100) {
          ...
          const chapterTitle = titleMatch ? titleMatch[1].trim() : `Capítulo ${chapterNumber}`;
    ...
    if (currentParagraph.length + sentence.length > 500 || 
      ...
      if (currentParagraph.trim()) {
        ...
        paragraphs.push({
          ...
          reference: `es_${chapter.bookCode} ${chapter.chapterNumber}.${paragraphNumber}`
        });
      }
...
async function parseAllSpanishBooks() {
  ...
  console.log(`📖 Parsing ${books.length} Spanish EGW books...\n`);
