...
var outputPath = process.argv[2] || 'data/timeline.json';
var url = 'https://raw.githubusercontent.com/BradyStephenson/bible-data/master/BibleData-Event.csv';
...
var extractBook = function (reference) {
    var match = reference === null || reference === void 0 ? void 0 : reference.match(/^([1-3]?\s?[A-Za-z]+)/);
        ...
        switch (_a.label) {
            ...
            case 3:
                ...
                _loop_1 = function (row) {
                    ...
                    var reference = record.event_reference_id || record.event_reference || '';
                    var book = extractBook(reference || record.event_location_reference_id || '');
                    if (!book)
                        return "continue";
                    var key = normalizeKey(book);
                    var entry = {
            ...
            case 5:
                _a.sent();
                console.log("Saved timeline data to ".concat(resolvedOutput));
                return [2 /*return*/];
        }
    });
}); })();
