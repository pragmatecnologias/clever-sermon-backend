...
function createEnglishMetadata() {
    ...
    var englishEpubs = files.filter(function (f) { return f.startsWith('en_') && f.endsWith('.epub'); });
    console.log("\uD83D\uDCDA Found ".concat(englishEpubs.length, " English EPUB files\n"));
    var books = englishEpubs.map(function (filename) {
        // Extract book code from filename (e.g., "en_PP.epub" -> "PP")
        var code = filename.replace('en_', '').replace('.epub', '');
        var info = ENGLISH_BOOK_INFO[code] || {
            title: code,
            category: 'Other'
        };
