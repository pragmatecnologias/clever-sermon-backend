"use strict";
/**
 * Parse Spanish EGW Books
 * Parses all Spanish EPUB files and extracts content
 */
...
var OUTPUT_DIR = path.join(__dirname, '../data/egw-parsed-spanish');
var METADATA_FILE = path.join(DOWNLOAD_DIR, 'metadata-spanish.json');
                ...
                for (_i = 0, zipEntries_1 = zipEntries; _i < zipEntries_1.length; _i++) {
                    ...
                    if (entry.entryName.endsWith('.xhtml') || entry.entryName.endsWith('.html')) {
                        ...
                        if (text.length > 100) {
                            ...
                            chapterTitle = titleMatch ? titleMatch[1].trim() : "Cap\u00EDtulo ".concat(chapterNumber);
                            chapters.push({
                                bookCode: bookCode,
                                bookTitle: bookTitle,
...
function splitIntoParagraphs(chapter) {
    ...
    for (var _i = 0, sentences_1 = sentences; _i < sentences_1.length; _i++) {
        ...
        if (currentParagraph.length + sentence.length > 500 ||
            (currentParagraph.length > 200 && sentence.match(/^[A-ZÁÉÍÓÚÑ]/))) {
            if (currentParagraph.trim()) {
                ...
                paragraphs.push({
                    ...
                    reference: "es_".concat(chapter.bookCode, " ").concat(chapter.chapterNumber, ".").concat(paragraphNumber)
                });
            }
            ...
            switch (_a.label) {
                case 0:
                    ...
                    if (!fs.existsSync(METADATA_FILE)) {
                        console.error('❌ Spanish metadata file not found. Run create-spanish-metadata.ts first.');
                        return [2 /*return*/];
                    }
                    metadata = JSON.parse(fs.readFileSync(METADATA_FILE, 'utf8'));
                    books = metadata.books;
                    console.log("\uD83D\uDCD6 Parsing ".concat(books.length, " Spanish EGW books...\n"));
                    allChapters = [];
                    allParagraphs = [];
                    successCount = 0;
                    failCount = 0;
                ...
                case 6:
                    ...
                    console.log('\n' + '='.repeat(60));
                    console.log('📊 Spanish Parsing Summary');
...
parseAllSpanishBooks().catch(console.error);
